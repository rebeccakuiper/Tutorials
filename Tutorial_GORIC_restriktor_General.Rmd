 ...
 * When one wish to use a list with constraints matrix (rhs, neq), it should be done as follows: `list(constraints = xxx, rhs = xxx, neq = x)`,     with `constraints`  being a matrix, `rhs` a vector (when not zero), and `neq` a number (when not zero), for which then holds: `constraints *     parameters >= rhs`, where the first `neq`-restrictions are equalities. Please also note the example below.
...
Moreover, we could have calculated the GORICA values and weights as well wchich is done as explained above:
...
Please note that the upcoming analyses can only be performed by means of the *GORICA*. It is because, the variable can be used for a broad range of models(e.g.,SEM models), and allow us for more modifications of parameters.
...
In order to demonstrate this, new data will be generated, in which the variables differ in their contribution to the dependent variable.
...
Then, one should standardardize the data, before one calculates the DV, since the parameters for continuous variables will be compared.
...
Currently, `comparison = "complement"` only works for one hypothesis and not for a whole set of hypotheses. Then, the complement of the hypothesis of interest is also evaluated. In case of one hypothesis, this is more powerful than including the unconstrained hypothesis.
...
The command `out` gives the results (i.e., per model / hypothesis the log-likelihood, penalty, GORIC(A), and GORIC(A)-weights).
...
If one is working with a lot of restrictions, and a lot of parameters, calculations can be slower, which has to do with the calculation of the level probability. In cases like this, there are some options that can help. One can, for example, use the bootstrap method instead (`mix.weights = "boot"`) and use multiple processors. In this case, code may look like the following example.
