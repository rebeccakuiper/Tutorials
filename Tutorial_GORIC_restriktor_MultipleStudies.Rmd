...
To see a more detailed overview of the data via descriptive statistics split by `group` variable, we could use the `describeBy()` function with `Monin$group` set to be a grouping variable, as follows:
```{r}
...
First, we need an R-object with unconstrained estimates, that is, in this example, the three group means and one residual variance.We specify the linear regression model using `lm()` function, as follows:
```{r}
...
On the other hand, if we wouldn't drop the intercept `y ~ group' would estimate an intercept, representing the mean of the reference group, and the mean differences between the other (here, two) groups and the reference group.
...
On the Monin data set, we are going to do an exploratory analysis, which means that we are going to use all combinations with equalities (and no restrictions). To specify all possibilities in a string format we need to remember the following:
...
```{r}
H00 <- 'group1 == group2 == group3' #also equivalent to 'group1 = group2 = group3'
H01 <- 'group1 == group2' #also equivalent to 'group1 = group2'
H02 <- 'group1 == group3' #also equivalent to 'group1 = group3'
H03 <- 'group2 == group3' #also equivalent to 'group2 = group3'
```
...
```{r, eval = FALSE}
set.seed(123) # Set seed value
goric(lm_fit_Monin,H00, H01, H02, H03, type = "gorica")
```
...
summary(output_repl)
```
Since the support for $H_1$ and $H_2$ is lower than for $H_u$, we can conclude that both are weak hypotheses. Hence, the study of Holubar did not replicate the findings of Monin.
...
#### Evidence synthesis
Before we can start with the evidence-synthesis, we need to set the type of evidence-synthesis. In this case, we will set it to 1 
('type_ev <- 1'), the added-evidence approach.
